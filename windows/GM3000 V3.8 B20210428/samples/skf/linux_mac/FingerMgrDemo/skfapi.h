...
#define SAR_ACTION_WAITING				0x0B000039	//等待签名按键
#define SAR_ACTION_CANCELED				0x0B000040	//动作取消,如等待签名按键超时
...
#define MAX_CONTAINER_NAME_LEN			39	//容器名最大长度
...
/*
*	生成解锁请求随机数, 该随机数用于生成解锁码, 在解锁之前不要拔掉设备
*	hApplication	[IN]应用句柄
*	pbRandom		[OUT] 返回的随机数
...
/*
*	使用ECC密钥协商算法，为计算会话密钥而产生协商参数，返回临时ECC密钥对的公钥及协商句柄 - 使用容器中的签名密钥对
*	hContainer		[IN] 容器句柄
*	ulAlgId			[IN] 会话密钥算法标识
...
/*
*	使用ECC密钥协商算法，产生协商参数并计算会话密钥，输出临时ECC密钥对公钥，并返回产生的密钥句柄 - 使用容器中的签名密钥对
*	hContainer					[IN] 容器句柄
*	ulAlgId						[IN] 会话密钥算法标识
...
/*
*	使用ECC密钥协商算法，使用自身协商句柄和响应方的协商参数计算会话密钥，同时返回会话密钥句柄 - 使用容器中的签名密钥对
